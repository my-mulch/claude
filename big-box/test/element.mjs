...
export default jest.suite(function () {
    ...
    this.expect(A.max({ axes: [0, 3] })).toEqual([["44", "36", "21"], ["21", "24", "11"]])
    ...
    this.expect(B.subtract({ with: B })).toEqual(bb.zerosLike({ tensor: B }))
    this.expect(C.slice({ region: [':', 0, ':'] }).min()).toEqual(1)
    this.expect(C.slice({ region: [':', 0, ':'] }).divide({ with: C.slice({ region: [':', 0, ':'] }) })).toEqual([["1", "1", "1"], ["1", "1", "1"], ["1", "1", "1"]])
