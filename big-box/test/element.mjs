...
export default jest.suite(function () {
    ...
    this.expect(new bb.mean({ of: B }).invoke()).toEqual("-15")
    this.expect(new bb.mean({ of: C }).invoke()).toEqual("16.66666603088379")
    this.expect(new bb.mean({ of: K }).invoke()).toEqual("4.525000095367432")
    this.expect(new bb.multiplication({ of: B, with: 6 }).invoke()).toEqual([["-276", "114"], ["-228", "54"], ["54", "-90"], ["-150", "-198"]])
    this.expect(new bb.min({ of: A }).invoke()).toEqual("-15")
    this.expect(new bb.min({ of: A, axes: [0, 3] }).invoke()).toEqual([["17", "21", "-15"], ["-2", "-13", "-5"]])
    this.expect(new bb.max({ of: A }).invoke()).toEqual("44")
    this.expect(new bb.max({ of: A, axes: [0, 3] }).invoke()).toEqual([["44", "36", "21"], ["21", "24", "11"]])
    this.expect(new bb.addition({ of: B, with: B }).invoke()).toEqual([[`${-46 * 2}`, `${19 * 2}`], [`${-38 * 2}`, `${9 * 2}`], [`${9 * 2}`, `${-15 * 2}`], [`${-25 * 2}`, `${-33 * 2}`]])
    this.expect(new bb.multiplication({ of: B, with: B }).invoke()).toEqual([[`${-46 * -46}`, `${-19 * -19}`], [`${-38 * -38}`, `${9 * 9}`], [`${9 * 9}`, `${-15 * -15}`], [`${-25 * -25}`, `${-33 * -33}`]])
    this.expect(new bb.subtraction({ of: B, with: B }).invoke()).toEqual(bb.zerosLike({ tensor: B }))
    this.expect(new bb.min({ of: C.slice({ region: [':', 0, ':'] }) }).invoke()).toEqual(1)
    this.expect(new bb.division({ of: C.slice({ region: [':', 0, ':'] }), with: C.slice({ region: [':', 0, ':'] }) }).invoke()).toEqual([["1", "1", "1"], ["1", "1", "1"], ["1", "1", "1"]])
    this.expect(new bb.mean({ of: B, axes: [1] }).invoke()).toEqual(["-13.5", "-14.5", "-3", "-29"])
    this.expect(new bb.mean({ of: B, axes: [0] }).invoke()).toEqual(["-25", "-5"])
