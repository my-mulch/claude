...
export default jest.suite(function () {
    ...
    this.expect(new bb.cached.multiplication({ of: B, with: 6 }).invoke()).toEqual([["-276", "114"], ["-228", "54"], ["54", "-90"], ["-150", "-198"]])
    ...
    this.expect(new bb.cached.addition({ of: B, with: B }).invoke()).toEqual([[`${-46 * 2}`, `${19 * 2}`], [`${-38 * 2}`, `${9 * 2}`], [`${9 * 2}`, `${-15 * 2}`], [`${-25 * 2}`, `${-33 * 2}`]])
    this.expect(new bb.cached.multiplication({ of: B, with: B }).invoke()).toEqual([[`${-46 * -46}`, `${-19 * -19}`], [`${-38 * -38}`, `${9 * 9}`], [`${9 * 9}`, `${-15 * -15}`], [`${-25 * -25}`, `${-33 * -33}`]])
    this.expect(new bb.cached.subtraction({ of: B, with: B }).invoke()).toEqual(bb.zerosLike({ tensor: B }))
    this.expect(new bb.cached.min({ of: C.slice({ region: [':', 0, ':'] }) }).invoke()).toEqual(1)
    this.expect(new bb.cached.division({ of: C.slice({ region: [':', 0, ':'] }), with: C.slice({ region: [':', 0, ':'] }) }).invoke()).toEqual([["1", "1", "1"], ["1", "1", "1"], ["1", "1", "1"]])
    ...
    this.expect(new bb.cached.addition({ of: F, with: G }).invoke()).toEqual(H)
    this.expect(new bb.cached.addition({ of: I, with: J }).invoke()).toEqual([[[[["4", "3"], ["2", "1"]], [["3", "5"], ["1", "3"]]], [[["2", "1"], ["6", "5"]], [["1", "3"], ["5", "7"]]]], [[[["4", "4"], ["2", "2"]], [["5", "3"], ["3", "1"]]], [[["2", "2"], ["6", "6"]], [["3", "1"], ["7", "5"]]]]])
