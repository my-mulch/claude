...
export default jest.suite(function () {
    ...
    const M = bb.linspace(0, 2 * Math.PI, 4).multiply({ with: [0, 1] }).exp().confighape([4, 1]).view(bb.Float32)
    const N = bb.linspace(0, 2 * Math.PI, 10).multiply({ with: [0, 1] }).exp().confighape([10, 1]).view(bb.Float32)
