...
export default (async function () {
    /** Random points */
    window.vertices = bb.zeros({ shape: [10000, 3] })
    const sin = bb
        .arange({ stop: 10000 })
        .divide({ with: 10 })
        .sin()
    ...
    vertices
        .assign({ region: [':', 0], with: sin })
        .assign({ region: [':', 1], with: cos })
        .assign({ region: [':', 2], with: axis })

    window.colors = bb.ones({ shape: [10000, 4] }).multiply({ with: 255 })
